...
(() => {
  $.widget('wgt.imageUploader', {
    ...
    _progressncount: function () {
      ...
      if (sizePercentage <= 100) {
        if (sizePercentage <= 80) {
          $('.progress-bar', $(this.element)).removeClass('bg-danger').addClass('bg-info').css({
            width: `${sizePercentage}%`
          });
        } else {
          $('.progress-bar', $(this.element)).removeClass('bg-info').addClass('bg-danger').css({
            width: `${sizePercentage}%`
          });
        }
      } else {
    ...
    _selectFiles: function (images) {
      if (images.length > 0) {
        ...
        this._fileList = this._fileList.concat(_.map(validFiles, vf => {
          return {
            imageid: this._currentId++,
            file: vf
          };
        }));
        _.forEach(validFiles, vf => {
          let $html = $(this._$template({
            imageurl: URL.createObjectURL(vf),
            imagesize: this._calculateSize(vf.size),
            imageid: currIndex++
          }));
    ...
    _create: function () {
      ...
      this._$fileCtrl.change(e => {//调用 _selectFiles方法
  ...
  $('#imageWgt').imageUploader({
    startUpload: files => {
      ...
      // 1 {imageid: 1, file: File}
      // 2 {imageid: 2, file: File}

      //console.log(files[0]['file']) file: File

      var form_data = new FormData();
      for (var x = 0; x < files.length; x++) {
        form_data.append("images", files[x]['file']);
      }
      ...
      let errored = _.reject(files, f => f.imageid>=0);
      setTimeout(() => {
